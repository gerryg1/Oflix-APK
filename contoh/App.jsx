...
function AppInner() {
  ...
  useEffect(() => {
    ...
    fetch(`/auth_api.php?action=verify&token=${encodeURIComponent(saved)}`)
      .then(r => r.json())
      .then(res => {
        if (res.ok && res.profile) { setProfile(res.profile); setToken(saved); }
        else localStorage.removeItem('oflix_token');
        setChecking(false);
      })
  ...
  useEffect(() => {
    function onSwitch() {
      if (token) fetch('/auth_api.php?action=switchProfile', {
        method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({token}),
      }).catch(()=>{});
      localStorage.removeItem('oflix_token');
      setProfile(null); setToken(null); nav('/');
    }
  ...
  useEffect(() => {
    function onLogout() {
      if (token) fetch('/auth_api.php?action=logout', {
        method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({token}),
      }).catch(()=>{});
      localStorage.removeItem('oflix_token');
      setProfile(null); setToken(null); nav('/');
  ...
  function handleLogin(prof, tok) { setProfile(prof); setToken(tok); }

  // Preloader once
  if (!showedPreloader) return <Preloader onDone={() => setShowedPreloader(true)} />;
  ...
  // No profile
  if (!profile) return <ProfilePicker onLogin={handleLogin} />;
