...
export default function VideoPlayer({
  ...
  useEffect(() => {
    ...
    function initAudio() {
      // Disabled on mobile to prevent the WebKit/iOS double-audio rendering bug.
      return;
    }
  ...
  useEffect(() => {
    const sizes = isFullscreen ? SUB_SIZES_LANDSCAPE : SUB_SIZES_PORTRAIT;
    const px = sizes[subSize] || (isFullscreen ? 16 : 24);
    ...
    s.textContent = `
      ...
      ::cue { font-family: 'NetflixSans', sans-serif !important; font-size: ${px}px !important; }
    `;
  }, [showCtrl, subSize, isFullscreen]);
      ...
      <div className={`player-ctrl ${showCtrl ? '' : 'player-ctrl--hidden'}`}>
        ...
        <div className="player-row-top" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', background: 'linear-gradient(to bottom, rgba(0,0,0,0.8) 0%, transparent 100%)' }}>
          ...
          <button className="pctrl-btn pctrl-back" style={{ padding: 8 }} onClick={(e) => {
            ...
            // Exit fullscreen / remove CSS rotation before closing
            try {
              if (document.fullscreenElement || document.webkitFullscreenElement) {
                (document.exitFullscreen || document.webkitExitFullscreen)?.call(document);
              }
